...
if(typeof adrinvoker == 'undefined'){
	...
	adrinvoker.call2 = function(svrName, args, isSync){
		var str = "成功执行原生服务adrinvoker.call2(name, args)\n"; 
		var str = "执行原生服务["+svrName+"]\n"; 
		str += "--name : " + svrName + "\n";
		str += "--args : " + args + "\n";
		str += "已成功执行！";
		$console.log(str);
	...
	adrinvoker.call = function(svrName, args, isSync){
		var str = "成功执行原生服务adrinvoker.call(name, args)\n"; 
		var str = "执行原生服务["+svrName+"]\n"; 
		str += "--name : " + svrName + "\n";
		str += "--args : " + args + "\n";
		str += "已成功执行！";
...
function UMP$UI$Mvc$Element$invoke(json){
	// json is {method:"xxx"}
	try{
		if(this._id){
			if($__isdebug()){
				...
				json["id"] = this._id;
				return $controls.invoke(json);
			}
		}else{
			alert("该控件的id为[" + this._id+"]，请检查");
		}
	}catch(e){
		alert("调用控件[" + this._id+"]的 invoke()方法出错");
	}
}
...
UMP.UI.Mvc.Element.prototype = {	
	...
	*/	
	...
	invoke : UMP$UI$Mvc$Element$invoke,
